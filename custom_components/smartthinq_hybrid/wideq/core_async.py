...
class CoreAsync:
    ...
    def _web_refresh_token_from_cookie(self) -> str | None:
        ...
        The LG membership server sometimes sends the ``refresh_token``
        cookie URL-encoded (e.g. containing literal ``%2B``/``%3D``
        sequences). Depending on the aiohttp version, the cookie jar may
        store that value verbatim instead of decoding it, which produces a
        broken refresh token that fails on the very next auth refresh call.
        Always run the raw cookie value through ``unquote`` so the token is
        normalized regardless of how the jar stored it.
        """
